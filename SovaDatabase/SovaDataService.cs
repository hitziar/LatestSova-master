...
namespace SovaDatabase
{
    public class SovaDataService : IDataService
    {
        ...
        public Posttype GetPosttype(int id)
        {
         ...
         public PagedList<Post> SearchInPosts(string w1)
        {
            using (var db = new SovaContext())
            {
                string paramentervalue = w1;
                var p1 = new MySqlParameter();
                p1.Value = paramentervalue;
                p1.ParameterName = "@1";


                //PagedList<Post> result = db.Database.ExecuteSqlCommand("search1(@1)", p1);

                //foreach (var post in result)
                //{
                //    Console.WriteLine("{0}, {1}", post.Title, post.Body);
                //}
                PagedList<Post> result = null;
